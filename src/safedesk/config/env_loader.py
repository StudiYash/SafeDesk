...
def load_environment(
    ...
    app_password = source.get(ENV_EMAIL_APP_PASSWORD, "")

    return EnvironmentSettings(
        safedesk_env=source.get(ENV_SAFEDESK_ENV, DEFAULT_ENVIRONMENT).strip() or DEFAULT_ENVIRONMENT,
        email_sender_address=source.get(ENV_EMAIL_SENDER_ADDRESS, "").strip(),
        email_app_password_present=bool(app_password.strip()),
        otp_receiver_email=source.get(ENV_OTP_RECEIVER_EMAIL, "").strip(),
        enable_real_email=parse_bool(source.get(ENV_ENABLE_REAL_EMAIL), default=False),
